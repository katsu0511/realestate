...
/**
 * Servlet implementation class RegisterAdditionalItemServlet
 */
@WebServlet("/register/additional_item")
public class RegisterAdditionalItemServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RegisterAdditionalItemServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("userName") == null || session.getAttribute("password") == null) {
			...
			String aditid = cib.getCreatedId("adit", "A", cmpnid);
			
			// aditidをセット
			request.setAttribute("aditid", aditid);
			
			request.getRequestDispatcher("/WEB-INF/app/register/register_additional_item.jsp").forward(request, response);
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("userName") == null || session.getAttribute("password") == null) {
			...
			try {
				...
				String sql1 = "SELECT * FROM adit WHERE cmpnid=? AND itemnm=?";
				...
				String sql2 = "SELECT * FROM adit WHERE cmpnid=? AND itemid=?";
				...
				if (row1 > 0 || row2 > 0) {
					...
					String[] adit = {item_name,item_id,max_lng,spc_lng,comment};
					request.setAttribute("errorMessage", errorMessage);
					request.setAttribute("aditid", adit_id);
					...
					// aditに保存
					String sql3 = "INSERT INTO adit(aditid,cmpnid,itemnm,itemid,maxlng,spclng,commnt) VALUES(?,?,?,?,?,?,?)";
					pstmt3 = conn.prepareStatement(sql3);
					pstmt3.setString(1, adit_id);
					pstmt3.setString(2, cmpn_id);
					pstmt3.setString(3, item_name);
					pstmt3.setString(4, item_id);
					pstmt3.setString(5, max_lng);
					pstmt3.setString(6, spc_lng);
					pstmt3.setString(7, comment);
